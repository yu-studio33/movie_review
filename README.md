...
Django製の映画レビューサイトです。TMDB(The Movie Database)
と連携し、最新の人気映画や検索結果をリアルタイムに表示。ユーザー登録・ログイン後、映画に対して星評価付きのレビューを投稿できます。
...
黒を基調としたダークテーマに、ピンク(#FF7FD4)
をアクセントカラーとして使用しています。シックな黒背景に映えるビビッドなピンクを差し色にすることで、洗練された印象を目指しました。フォントはヘッダーロゴにArchivo
Narrow、本文にNoto Sans JPを使用しています。トップページは「あなたへのおすすめ」「新着映画」「人気の映画」をセクションごとに分け、主要動画配信サービスのUI設計を参考にしています。
> > > > > > > 28e1250 (Add screenshot to README)
...
日本語SNS投稿の感情分析データセット「WRIME」を用いて、Janomeによる形態素解析とTF-IDFによるベクトル化、ロジスティック回帰による学習を行いました(
テストデータでの精度: 81%)。学習済みモデルは`movies/ml_models/`
に格納され、レビュー投稿時に自動でポジティブ/ネガティブを判定します。この判定結果をもとに、協調フィルタリング(
似た好みのユーザーのポジティブ評価を集計する手法)で「あなたへのおすすめ」を算出しています。

デモ用データ投入コマンド(`seed_reviews`)
は、映画のジャンルに応じたコメント文をカテゴリ別に用意し、同一映画内で文言が重複しないよう管理する仕組みを備えており、12人のテストユーザー・16作品にまたがる自然なレビューデータを自動生成します。
